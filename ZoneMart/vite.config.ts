...
function parseAndRecordTransactionText(text: string): boolean {
  ...
  const bankMatch = text.match(
    /(?:TK|STK|tài khoản)[:\s-]+(?:\w+\s*-\s*)?(\d{8,16})/i,
  );
  if (bankMatch) {
...
function fetchDiscordMessages(
  token: string,
  channelId: string = '1548932658388668438',
): Promise<any[]> {
...
async function syncDiscordMessages() {
  try {
    const msgs = await fetchDiscordMessages(
      cachedDiscordToken,
      '1548932658388668438',
    );
    for (const m of msgs) {
...
function setupDiscordGateway(token: string) {
  try {
    ...
    ws.onopen = () => {
      console.log(
        '>>> [DISCORD GATEWAY CONNECTED] Lắng nghe thông báo VietQR từ Discord thời gian thực...',
      );
    };
    ...
    ws.onmessage = (event) => {
      try {
        ...
        if (t === 'MESSAGE_CREATE' && d && d.content) {
          ...
          if (
            text.includes('ZM') ||
            text.includes('TPBank') ||
            text.includes('28122068866')
          ) {
      ...
      function setupVietQrWs() {
        try {
          const ws = new WebSocket(
            'wss://api.vietqr.org/vqr/socket?clientId=customer-zonemart-user26685',
          );
          ws.onopen = () =>
            console.log(
              '>>> [SERVER VIETQR WEBSOCKET CONNECTED] Sẵn sàng tự động nhận tiền thật',
            );
          ws.onmessage = (e) => {
            try {
              ...
              const code =
                extractOrderCode(combinedText) ||
                (data.orderId ? String(data.orderId).toUpperCase() : '');
              if (code) {
                const amount = Number(data.amount) || undefined;
                const bankAccount = String(
                  data.bankaccount || data.accountNo || '',
                );
      ...
      server.middlewares.use(
        async (req: IncomingMessage, res: ServerResponse, next: () => void) => {
          ...
          if (pathname === '/api/payment/check' && req.method === 'GET') {
            ...
            const cleanCode =
              extractOrderCode(rawCode) || rawCode.toUpperCase().trim();
            ...
            for (const [key, val] of paidOrdersCache.entries()) {
              ...
              if (
                cleanCode &&
                (key.includes(cleanCode) || rawStr.includes(cleanCode))
              ) {
                res.end(
                  JSON.stringify({
                    paid: true,
          ...
          if (pathname === '/api/payment/tunnel-info' && req.method === 'GET') {
            ...
            res.end(
              JSON.stringify({
                ...
                tokenUrl:
                  'https://monogram-dilute-tightly.ngrok-free.dev/api/token_generate',
                callbackUrl:
                  'https://monogram-dilute-tightly.ngrok-free.dev/bank/api/transaction-sync',
                totalPaidRecorded: paidOrdersCache.size,
                recentTransactions: Array.from(paidOrdersCache.entries()).slice(
                  -5,
                ),
              }),
            );
            ...
            req.on('end', () => {
              try {
                ...
                if (
                  ...
                  const matchedCode =
                    extractOrderCode(searchScope) ||
                    (body.orderId ? String(body.orderId).toUpperCase() : '');
                  const bankAccount = String(
                    body.bankaccount ??
                      body.bankAccount ??
                      body.accountNo ??
                  ...
                  if (matchedCode) {
                    ...
                    savePaidOrders(paidOrdersCache);
                    console.log(
                      `>>> [TIỀN VÀO KHÔNG RÕ MÃ ĐƠN] Số tiền: ${amount}đ, Lưu tạm: ${genericKey}`,
                    );
                  }
